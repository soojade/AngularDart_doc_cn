...
我们必须通过注册一个`HeroService`**提供商**来告诉*注入器*怎样创建`HeroService`实例。要做到这一点，通过在组件元数据`@Component`注解底部添加如下的`providers`参数：
...
仔细想一下，我们应该在哪里、什么时候注册提供商（provider），理解注册的作用域。小心不要在错误的层上创建一个新的实例。
