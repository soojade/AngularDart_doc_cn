...
从*英雄之旅*中构建的`HeroesComponent`的简化版本开始。
...
我们必须通过注册**提供者**(provider)来配置注入器，这些提供者为应用创建所需服务。在本章的稍后部分会解释什么是提供者。

在此之前，让我们看一个在引导期间注册提供者的例子：
...
首选的方法是在应用的组件注册应用提供者。因为`HeroService`在*英雄*特性区域可用，其它地方不可用，理想的地方是在顶级的`HeroesComponent`组件内注册。
...
无论在《英雄之旅》还是其它范例中，都没有出现这样的代码。 在必要时，可以写使用显式注入器的代码，但却很少这样做。当 Angular 创建组件时——无论通过像`<hero-list></hero-list>`这样的 HTML 标签还是通过路由导航到组件——它都会自己管理好注入器的创建和调用。只要让 Angular 做好它自己的工作，我们就能安心享受自动依赖注入带来的好处。
...
Angular 告诉我们，依赖注入器找不到日志服务的*提供者*。它需要提供者来创建一个`Logger`实例来注入到一个新的`HeroService`实例，而`HeroService`又需要创建并注入到`HeroListComponent`的新实例。

这个“创建链”始于`Logger`的提供者。这个*提供者*就是下一节的主题。

<h3 id="provider">注入器提供者</h3>

提供者提供依赖值的一个具体的、运行时的版本。注入器依靠**提供者**创建服务的实例，注入器再将服务的实例注入组件或其它服务。

我们必须为注入器注册一个服务的*提供者*，否则它不知道该如何创建该服务。
...
有很多方式可以提供一些实现 `Logger`的东西。`Logger`类本身是一个显而易见而且自然而然的提供者。 但它不是唯一的选项。

我们可以用其它提供者来配置注入器，只要它能交付一个`Logger`。我们可以提供一个替代类。我们可以给它一个提供者，让它调用可以创建日志服务的工厂函数。所有这些方法，只要用在正确的场合，都可能是一个好的选择。

最重要的是，当注入器需要一个`Logger`时，它得先有一个提供者。
...
这实际上是用于注册提供者的简写方式，完整的是创建一个`Provider`类的新实例：
...
第一个是令牌 (token)，它作为键值 (key) 使用，用于定位依赖值和注册提供者。
...
于是可以通过`useValue`选项来注册提供者，它会让这个对象直接扮演 `logger` 的角色。
...
我们可以注入`Logger`，但是不能注入布尔型的`isAuthorized`。我们不得不通过通过工厂提供者创建这个`HeroService`的新实例。
...
我们同时把`Logger`和`UserService`注入到工厂提供者中，并且让注入器把它们传给工厂方法：
...
> `useFactory`字段告诉 Angular：这个提供者是一个工厂方法，它的实现是`heroServiceFactory`。
>
> `deps`属性是*提供者令牌*列表。`Logger`和`UserService`类作为它们自身类提供者的令牌。注入器解析这些令牌，把相应的服务注入到工厂函数中相应的参数中去。

注意，我们在一个常量中捕获了这个工厂提供者：`heroServiceProvider`。这个额外的步骤让工厂提供者可被复用。无论哪里需要，都可以使用这个变量注册`HeroService`。
...
当我们向注入器注册提供者时，实际上是把这个提供者和一个 DI 令牌关联起来了。 注入器维护一个内部的令牌-提供者映射表，这个映射表会在请求依赖时被引用到。令牌就是这个映射表中的键值。
...
我们想让这个配置对象在注入时可用，而且知道可以使用值提供者来注册一个对象。

但是，这种情况下用什么作令牌呢？虽然我们可以使用*Map*，然而却不能，因为`Map`太普通了。我们的应用可能依赖几个*maps*，每一个都有不同的目的。

> 在Typescript中， 接口不能作为提供者令牌。Dart 没有这一限制；每一个类隐式的定义了接口，所以接口名就是类名。`Mpa`是可用的令牌，即使它是抽象类的名称；仅仅因为它太普通了，不适合做令牌。
...
如果我们使用如果我们使用级联符号，配置对象不能被声明为`const`，并且我们不能使用值提供者。一个解决方案是使用工厂提供者。我们下次说明。我们也展示了在我们顶级的`AppComponent`组件中怎样提供和注入配置对象：
...
本章，我们学习了 Angular 依赖注入的基础知识。我们可以注册很多种类的提供者，知道如何通过添加构造函数的参数来请求一个注入对象（例如一个服务）。
